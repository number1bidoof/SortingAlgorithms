...
public class Exercise23_04 {
    ...
    private static int partition(int[] list, int first, int last) {
        ...
        int mid = list[(first + last) / 2]; // median of the indexes

        if (last - first < 2) { // edge case, skips if there are not enough elements in the array
            // finds the median of first, middle & last
            if ((first < mid && mid < last) || (last < mid && mid < first)){
                // in this case, number is moved to the median, if the median is in between the upper (first) & lower (last) bound
                int temp = list[first];
                list[first] = list[mid];
                list[mid] = temp;

            } else if ((mid < first && first < last) || (last < first && first < mid)){
                // the number is moved to the first number, as the upper bound is greater than the median
                int temp = list[first];
                list[first] = temp;

            } else{
                // the number is moved to the last number, if the given number isn't in between or greater than the provided
                int temp = list[first];
